...
export function MarqueePills() {
  ...
  return (
    <>
      ...
      <div className="rounded-[1.4rem] border border-white/12 bg-[linear-gradient(180deg,rgba(20,31,53,0.88),rgba(12,20,38,0.94))] p-4 shadow-[0_20px_56px_rgba(4,8,24,0.22)] backdrop-blur-md sm:p-5">
        <div className="flex flex-col gap-3 text-center">
          <p className="text-[11px] font-semibold uppercase tracking-[0.24em] text-cyan-100/80">Choose your destination</p>
          <p className="text-sm leading-6 text-slate-200">Pre-select the destination so readiness, pricing, and the application builder start from the right country profile.</p>
        </div>
        <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-3">
          {curatedDestinations.map((destination) => (
            <button
              ...
              className={`group relative overflow-hidden rounded-[1.15rem] border border-white/12 bg-slate-900/75 px-4 py-3 text-left text-slate-200 shadow-[0_16px_32px_rgba(8,15,35,0.16)] transition hover:-translate-y-0.5 hover:border-blue-400/55 hover:text-white ${destination.glowClassName}`}
            >
