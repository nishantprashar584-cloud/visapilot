...
function buildCoverLetterAutogenerationKey(applicant: ApplicantInfo): string {
  return JSON.stringify({
    firstName: applicant.personal.firstName,
    lastName: applicant.personal.lastName,
    ...
    returnIntentEvidence: applicant.homeTies.returnIntentEvidence,
    dependentInformation: applicant.homeTies.dependentInformation,
    residenceCountry: applicant.contact.residenceCountry,
    placeOfApplication: applicant.application.placeOfApplication,
  });
}
...
export function ApplicationWizard({
  ...
  useEffect(() => {
    const sanitizedDraft = stripItineraryMatrixSection(coverLetterDraft);

    if (sanitizedDraft !== coverLetterDraft) {
      setCoverLetterDraft(sanitizedDraft);
    }
  }, [coverLetterDraft]);

  useEffect(() => {
    if (!hasHydrated || currentStep !== 4 || isGeneratingCoverLetter) {
      return;
    }

    if (stripItineraryMatrixSection(coverLetterDraft).trim()) {
      return;
    }

    const applicant = form.getValues();
    const generationKey = buildCoverLetterAutogenerationKey(applicant);

    if (autoGeneratedCoverLetterKeyRef.current === generationKey) {
      return;
    }

    autoGeneratedCoverLetterKeyRef.current = generationKey;
    void handleGenerateCoverLetter(applicant);
  }, [coverLetterDraft, currentStep, form, hasHydrated, isGeneratingCoverLetter, watchedValues]);
